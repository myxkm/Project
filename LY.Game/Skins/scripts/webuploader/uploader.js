    ...
    $.fn.InitUploader = function (b) {
        var fun = function (parentObj) {
           var p = $.extend({}, $.upLoadDefaults.property,b || {});
            ...
            if (!p.multiple) {
                p.sendurl += "&D_Path=" + parentObj.siblings(".upload-path").val();
            }
            ...
            uploader.on('fileQueued', function (file) {
                //如果是单文件上传，把旧的文件地址传过去
                if (!p.multiple) {
                    uploader.options.formData.D_Path = parentObj.siblings(".upload-path").val();
                }
                //防止重复创建
                ...
                if (data.status == '1') {
                    ...
                    if (!p.multiple) {
                        if (p.thumbnail) {
                            ...
                            parentObj.siblings(".cover").children("img").attr("src",data.thumb);
                        }
                        else { parentObj.siblings(".upload-path").val(data.path); }
